...
| `action` | `string` | Filter by action type | `LOGIN`, `BATCH_IMPORT`, `SHEET_UPDATE` |
| `resource_type` | `string` | Filter by resource type | [user](file:///workspaces/omr-backend/src/api/routers/auth.py#136-152), [batch](file:///workspaces/omr-backend/src/api/routers/batches.py#719-766), `sheet`, `task` |
...
## UI Recommendations
1. **Admin Dashboard**: Add an "Audit Logs" tab.
2. **Detail Views**:
...
3. **Diff View**: The `details` field (and `old_values`/`new_values` in backend) can be used to show what changed. For now, a simple JSON view or "Field X changed from A to B" is sufficient.
